...
We minimize using L-BFGS-B with analytically computed gradients.
The standard KRR solution (all $\mu_j = 0$) provides a warm start that places the optimizer in the basin of attraction of the theory-informed minimum.
In practice, we use L-BFGS-B for all penalty types, as the analytical Hessians required by the augmented linear system approach are computationally expensive for the full restriction set.
Convergence requires 50--200 L-BFGS iterations, each costing $O(n^2)$ for the gradient computation via analytically derived penalty gradients.
...
\paragraph{Software.}
The estimator is implemented in Python using NumPy and SciPy for linear algebra.
Kernel matrices are constructed via \texttt{scipy.spatial.distance}, the Cholesky factorization uses \texttt{scipy.linalg.cho\_factor}, and L-BFGS-B optimization uses \texttt{scipy.optimize.minimize}.
Baseline models (OLS, Ridge, Lasso, Elastic Net, Random Forest) use scikit-learn; the neural network baseline uses PyTorch.
