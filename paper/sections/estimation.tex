...
\paragraph{Three-way temporal split.}
Following the sample-splitting convention of \citet{gu2020empirical}, we partition the time series into three non-overlapping periods:
\begin{enumerate}
\item \textbf{Training sample} (1960--1986): estimate $f(\cdot)$ and all structural parameters $\hat{\theta}_j$ for each candidate hyperparameter configuration.
\item \textbf{Validation sample} (1987--2000): select the hyperparameter vector $(\hat{\lambda}_{\mathrm{stat}}, \hat{\bm{\mu}}^{\mathrm{group}})$ that minimizes average squared prediction error:
...
\item \textbf{Testing sample} (2001--present): evaluate out-of-sample performance using the selected hyperparameters. This sample is never used for estimation or tuning.
\end{enumerate}
The 1987 validation cutoff follows the convention in \citet{gu2020empirical} and ensures a 27-year training window. The three-way split provides a clean separation: the training sample determines $f$ and $\hat{\theta}_j$, the validation sample tunes $(\lambda_{\mathrm{stat}}, \bm{\mu})$, and the testing sample evaluates OOS performance without contamination from hyperparameter selection.
...
\paragraph{Quadratic penalties (Types A and B).}
