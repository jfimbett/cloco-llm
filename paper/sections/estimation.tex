...
Our baseline uses calibration (Approach~3) for parameters with established values and pre-estimation (Approach~2) for SDF loadings, following the standard practice in the empirical asset pricing literature.
Joint estimation (Approach~1) is reserved for robustness analysis, where we verify that the theory ranking is qualitatively stable across estimation approaches.
...
We search the 9-dimensional hyperparameter space using adaptive coordinate descent over the group multipliers, an approach inspired by the cyclic coordinate descent algorithm that underlies the Lasso and elastic net solvers of \citet{friedman2010regularization}.
...
Holding $\lambda_{\mathrm{stat}}$ fixed, we then optimize the eight group multipliers in two stages.
In the coarse stage, we cycle through each group $g = 1, \ldots, G$ and perform a one-dimensional line search over a log-spaced grid:
\begin{equation}\label{eq:cd_grid}
\mu_g^{\mathrm{group}} \in \{0\} \cup \{10^{u} : u \in \{-4, -3, -2, -1, 0, 1, 2\}\}.
\end{equation}
In the fine stage, the grid is refined adaptively around each group's current value: if $\mu_g > 0$, the search concentrates on $\{0\} \cup \{10^u : u \in \mathrm{linspace}(\log_{10}\mu_g - 1, \log_{10}\mu_g + 1, 10)\}$; if $\mu_g = 0$, it searches $\{0\} \cup \{10^u : u \in \mathrm{linspace}(-6, -2, 10)\}$.
For each candidate value, all other multipliers are held at their current values, and the validation-set MSE is evaluated using a short L-BFGS optimization (15 iterations) that adjusts the Nystr\"{o}m coefficient vector $\boldsymbol{\beta}$ to account for the structural penalties.
The value that yields the lowest MSE is retained, and the algorithm moves to the next group.
The fine stage terminates early if no group multiplier changes in a complete cycle.

The total number of function evaluations is at most $2 \times 8 \times 11 = 176$, compared to $k^9$ for an exhaustive grid.
Each evaluation requires solving one Nystr\"{o}m-approximated KRR system (an $m \times m$ Cholesky factorization with $m = 500$; see Appendix~\ref{app:nystrom}), followed by L-BFGS refinement that incorporates the structural penalty gradient.
Because the candidate values within each group are independent, they are evaluated in parallel across CPU cores, so each group's line search completes in the wall-clock time of a single evaluation.
...
When structural penalties are active, we optimize the Nystr\"{o}m coefficient vector $\boldsymbol{\beta}$ via L-BFGS, initialized from the standard KRR warm start.
Specifically, let $\hat{\boldsymbol{\beta}}_{\mathrm{KRR}}$ denote the Nystr\"{o}m KRR solution (all $\mu_j = 0$).
We minimize the penalized objective
\begin{equation}\label{eq:lbfgs_obj}
L(\boldsymbol{\beta}) = \|\mathbf{y} - \mathbf{Z}\boldsymbol{\beta}\|^2 + n\lambda_{\mathrm{stat}}\|\boldsymbol{\beta}\|^2 + \sum_j \mu_j\, C_j(\mathbf{Z}\boldsymbol{\beta})
\end{equation}
using L-BFGS with gradient~\eqref{eq:nystrom_gradient}, starting from $\hat{\boldsymbol{\beta}}_{\mathrm{KRR}}$.
During the coordinate descent's hyperparameter search, we run L-BFGS for 15 iterations per candidate configuration to obtain a fast approximate evaluation of validation-set MSE.
For the final out-of-sample prediction (after $\bm{\mu}$ is selected), we run L-BFGS for 200 iterations to convergence.
This approach is computationally efficient because the L-BFGS iterates operate on the $m$-dimensional coefficient vector $\boldsymbol{\beta}$ rather than the $n$-dimensional $\boldsymbol{\alpha}$, and the KRR warm start ensures rapid convergence.
