...
Theory-KRR outperforms standard KRR in out-of-sample return prediction, with meaningful $R^2_{\text{OOS}}$ gains and Sharpe ratio improvements when the penalty multipliers are selected by cross-validation.
...
Our eight-group cross-validation aggregates heterogeneity within groups---estimating individual $\mu_j$ for all fifty-six restrictions would sharpen the ranking at the cost of a larger hyperparameter search.
