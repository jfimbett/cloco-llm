...
We compare Theory-KRR against eight baseline models spanning the spectrum from simple linear methods through polynomial expansions to a flexible nonparametric estimator.
...
All models use the full set of firm-level characteristics and macroeconomic state variables as predictors.
...
\item \textbf{Polynomial models (degree 2).}
Ridge, Lasso, and Elastic Net applied to degree-2 polynomial features: for each predictor $X_k$, we include $X_k^2$ and all pairwise interactions $X_k X_l$ ($k < l$).
This expansion captures nonlinearities and interactions within a linear estimation framework, providing a benchmark for whether the kernel's implicit infinite-dimensional feature map adds value beyond explicit low-order interactions.
...
We estimate two variants of Theory-KRR that differ in the role of statistical regularization:
\begin{itemize}
\item \textbf{Theory-KRR ($\lambda = 0$):} The statistical penalty $\lambda_{\mathrm{stat}}$ is set to zero, so the model relies entirely on the structural penalties $\mu_j C_j(f)$ for regularization.
The cross-validation procedure selects the multiplier vector $\bm{\mu}$ subject to the constraint that at least one $\mu_g > 0$ (pure theory-guided shrinkage).
\item \textbf{Theory-KRR ($\lambda = \lambda^*$):} Both the statistical penalty $\lambda_{\mathrm{stat}}$ and the structural multipliers $\bm{\mu}$ are selected jointly by cross-validation.
This is the fully flexible specification that nests standard KRR as a special case (when all $\mu_j = 0$).
\end{itemize}
