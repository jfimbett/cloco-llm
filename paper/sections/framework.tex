...
Sections~\ref{sub:penalties}--\ref{sub:finite_dim} formalize the three structural penalty types
and establish the representer theorem and finite-dimensional reduction that
make the estimator computationally tractable.
Section~\ref{sub:nesting} shows that the framework nests standard KRR
and constrained GMM as special cases.
...
We formalize three classes of structural penalties.
...
Type~A penalties accommodate any model that specifies an SDF $M_{j,t+1}$.
This includes consumption-based models (CCAPM, long-run risk,
rare disasters, habit formation), intermediary-based models where the SDF
is linear in a factor $F_{j,t}$ (i.e., $M_{j,t+1} = a_j + b_j F_{j,t+1}$,
as in \citealp{hekrishnamurthy2013} and \citealp{adrianetulamuir2014}),
information and learning models, and behavioral models that specify
a modified pricing kernel.
A linear factor model $M_{t+1} = 1 - \mathbf{b}^\top(\mathbf{F}_{t+1} - \E[\mathbf{F}])$
is a linear SDF, so any factor-based restriction reduces to a Type~A
Euler equation penalty without requiring pre-estimated betas---only
the factor time series $F_{j,t}$ is needed.
...
%----------------------------------------------------------------------
\subsubsection{Type~C: Demand-System Equilibrium Penalties}
\label{sub:demand}
%----------------------------------------------------------------------
...
Type~C penalties encode the equilibrium restriction that aggregate demand
...
\begin{remark}[Verification for Types~A, B, and~D]
\label{rem:verify_representer}
The condition~\eqref{eq:penalty_eval} holds for all three penalty types:
...
\begin{remark}[Evaluation-dependence and variable coverage]
\label{rem:evaluation_dependence}
A key feature of the penalty structure is that each $C_j(f)$ constrains
$f$ only through its evaluations at the observed data points
$\{f(\mathbf{X}_s)\}_{s=1}^n$.
This has an important implication for the interpretation of penalties
that involve only a subset of the characteristic vector $\mathbf{X}$.
Consider a production-based penalty $C_j^{\mathrm{prod}}(f)$ that depends
on investment-to-capital ratio $(I/K)$ and return on equity $(\mathrm{ROE})$
but not on consumption growth.
This penalty does \emph{not} impose a zero restriction on $f$'s dependence
on consumption---it is simply silent about consumption.
The penalty penalizes deviations of $f(\mathbf{X}_s)$ from the production
model's prediction $g_j(\mathbf{Z}_s^{(j)})$ at each observed point,
but it leaves $f$ free to depend on any variable in $\mathbf{X}$
to match those targets.
More generally, each $C_j$ constrains $f$ along specific dimensions
of the characteristic space while leaving other dimensions unrestricted.
The combined penalty $\sum_j \mu_j C_j(f)$ achieves broad coverage
by assembling restrictions from different models, each constraining
different dimensions of $\mathbf{X}$.
\end{remark}
...
\label{prop:foc}
Suppose every active penalty ($\mu_j > 0$) is of Type~A or~B
...
\end{equation}
where $\mathcal{J}^A$ and $\mathcal{J}^B$ index
Type~A and B penalties respectively, and the aggregated target vector
\begin{equation}
\label{eq:omega_vec}
\boldsymbol{\omega} \;=\;
...
\begin{remark}[Type~C penalties and convexity]
...
\begin{remark}[Uniqueness under nonconvex penalties]
\label{rem:uniqueness_nonconvex}
When some $C_j$ are nonconvex in the fitted values---as arises with
nonlinear SDF specifications in Type~A penalties---the overall
objective~\eqref{eq:finite_dim} may have multiple local minima.
A sufficient condition for uniqueness is that the statistical
regularization dominates the nonconvex curvature:
if the spectral norm of the Hessian of
$\sum_j \mu_j \tilde{C}_j(\hat{\mathbf{f}})$ at any critical point
is bounded by $2\lambda_{\mathrm{stat}}\,\lambda_{\min}(\mathbf{K})/n$,
where $\lambda_{\min}(\mathbf{K})$ is the smallest eigenvalue of
$\mathbf{K}$, then the overall Hessian remains positive definite and
the solution is unique.
This condition is verifiable in practice from the kernel matrix
and the penalty structure.
The representer theorem (Proposition~\ref{prop:representer})
holds regardless of convexity---it requires only that $\lambda_{\mathrm{stat}} > 0$
and that penalties depend on $f$ through evaluations---so the
finite-dimensional reduction is valid even when the reduced problem
has multiple critical points \citep{bodin2024random}.
\end{remark}
...
The framework nests two important special cases, clarifying
the relationship between theory-informed KRR and existing methods.
...
the three penalty types (A, B, D) each potentially with multiple variants
(e.g., consumption-based vs.\ intermediary-based SDFs within Type~A).
...
\begin{remark}[Misspecification and variable coverage]
\label{rem:misspec_variables}
Consider a concrete case: the investment CAPM (Restriction~14) predicts expected
returns as a function of investment-to-capital ratio and profitability, but
says nothing about consumption growth.
Does this mean the penalty implicitly imposes that consumption has zero effect?
No.
Each $C_j$ constrains $f$ along the specific dimensions of $\mathbf{X}$ that
enter the model's predictions, but leaves $f$'s dependence on other variables
entirely unrestricted.
The investment penalty pushes $f(\mathbf{X}_s)$ toward the production model's
prediction $g_j(I/K, \mathrm{ROE})$ at each observation $s$, but $f$ remains
free to use consumption growth, sentiment, or any other variable in $\mathbf{X}$
to satisfy that target.
Silence about a variable is not the same as imposing it has zero effect.
The combined penalty $\sum_j \mu_j C_j(f)$ covers many dimensions of the
characteristic space through different restrictions, and the $\mu_j \to 0$
mechanism (Proposition~\ref{prop:misspecification}) handles restrictions
that are quantitatively wrong, not merely incomplete.
\end{remark}
...
\begin{remark}[Structural penalties and the saturation barrier]
\label{rem:saturation}
Standard KRR is minimax-optimal when the target function $f^*$ satisfies
a source condition $f^* \in [\mathcal{H}]^s$ with $0 < s \leq 1$,
but saturates for smoother targets: when $s > 1$, the convergence rate
cannot improve beyond $n^{-2/(2+\beta)}$ regardless of how
$\lambda_{\mathrm{stat}}$ is tuned, where $\beta$ governs the eigenvalue
decay of the kernel integral operator \citep{li2023saturation,zhang2025optimal}.
This saturation effect is intrinsic to all spectral regularization methods,
which lack the feature-learning capability of adaptive estimators
\citep{barzilai2024spectral}.
The structural penalties $\sum_j \mu_j C_j(f)$ can partially mitigate
saturation by reducing the effective function class from the full RKHS
$\mathcal{H}$ to a smaller set consistent with economic theory.
When the penalties correctly narrow the search space, the effective
source condition improves and the saturation boundary shifts upward.
In the geometrical framework of \citet{zhang2024geometrical},
this corresponds to projecting the estimator onto a lower-dimensional
subspace of $\mathcal{H}$ compatible with the active structural
restrictions, reducing the effective estimation dimension and
...
\begin{remark}[Asymptotic foundations for $\mu_j$ inference]
\label{rem:fischer_steinwart}
The recent results of \citet{fischer2024asymptotic} provide a rigorous
asymptotic theory for linear functionals of KRR estimators.
For any functional $\ell(f) = \langle f, g \rangle_{\mathcal{H}}$
with $g \in \mathcal{H}$, they establish a central limit theorem:
the standardized functional converges to $\mathcal{N}(0,1)$ under
appropriate regularization.
Point evaluations achieve variance $O(n^{-1}\lambda^{-d/m})$ and
bias $O(\lambda^{1-d/(2m)})$, where $d$ is the input dimension and
$m$ the kernel smoothness order.
The optimal $\lambda \sim n^{-1}$ yields a parametric rate for
sufficiently smooth functionals.
Since the gradient of $C_j$ evaluated at $\hat{f}$ is a linear
functional of $\hat{f}$, these results support a score-type test
for $H_0: \mu_j = 0$: evaluate the derivative
$\partial C_j(f)/\partial f$ at the unconstrained KRR solution
$\hat{f}^{\mathrm{KRR}}$, and reject when this gradient is large
relative to its asymptotic standard error.
This provides an alternative to the permutation-based approach
of Remark~\ref{rem:permutation}, grounded in distributional theory
rather than resampling.
For uniform inference across evaluation points,
\citet{singh2024kernel} constructs valid confidence bands
using a Gaussian multiplier bootstrap calibrated to the RKHS norm.
\end{remark}
