...
description: Draft pre-analysis plans by dispatching the causal-strategist agent in PAP mode. Covers AEA RCT Registry, OSF, and EGAP standards. Specifies outcomes, subgroups, multiple testing, power calculations, and exclusion rules.
...
Draft a pre-analysis plan by dispatching the **causal-strategist** agent in PAP mode.
...
### Step 2: Launch causal-strategist Agent (PAP Mode)

Delegate to the `causal-strategist` agent via Task tool:
...
### Step 4: Launch econometrics-critic Review (Optional)

If the PAP includes a complex identification strategy, optionally dispatch the econometrics-critic:
...
- **causal-strategist creates, econometrics-critic critiques** (optional but recommended for complex designs).
