---
name: proofread
description: Manuscript proofreading dispatching the academic-proofreader agent. Checks 6 categories — structure, claims-evidence alignment, identification fidelity, writing quality, grammar, and compilation. Produces report without editing files.
...
Run the proofreading protocol by dispatching the **academic-proofreader** agent.
...
### Step 2: Launch academic-proofreader Agent

Delegate to the `academic-proofreader` agent via Task tool:
...
The academic-proofreader applies deductions (0-100 scale):
...
- Three-strikes escalation (if applicable):
  ...
  - Strategy misrepresented → escalate to causal-strategist
...
- **academic-proofreader is a CRITIC, not a creator.** It never writes or revises.
