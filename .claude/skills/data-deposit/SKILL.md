---
name: data-deposit
description: Prepare AEA Data Editor compliant replication package by dispatching Coder (main Claude) for package assembly and replication-verifier agent for validation. Generates README, master script, numbered script order, and deposit checklist.
...
Prepare an AEA Data Editor compliant replication package by dispatching the **Coder (main Claude)** (assembly) and **replication-verifier** (validation).
...
Delegate to the Coder (main Claude):
...
### Step 3: Validate with replication-verifier

After package assembly, dispatch the **replication-verifier** in submission mode:
...
If replication-verifier finds failures:
1. Re-dispatch Coder with specific fixes (max 3 rounds)
2. Re-run replication-verifier to verify
...
- **Worker-critic pairing.** Coder assembles, replication-verifier validates.
