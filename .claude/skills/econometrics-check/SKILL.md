...
description: Causal inference design audit. Dispatches the econometrics-critic agent in standalone mode for 4-phase review (claim, design validity, inference, polish). Covers DiD, IV, RDD, Synthetic Control, and Event Studies. Use when working on empirical papers, strategy memos, or R scripts with causal estimators.
...
Run a 4-phase causal inference audit on the target file(s) by dispatching the **econometrics-critic** agent in standalone mode.
...
2. Read `paper/references.bib` to check citation availability
...
### Step 3: Launch econometrics-critic Agent

Delegate to the `econometrics-critic` agent via Task tool:
...
Report the econometrics-critic's score. In standalone mode, this is the component score (not the weighted aggregate). Reference `scoring-protocol.md` for how this feeds into the overall score when run within the pipeline.
