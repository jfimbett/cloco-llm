...
# Files that need verification — value is the reminder action
VERIFY_EXTENSIONS = {
...
# Extra reminder for files in these directories (appended after main action)
EXTRA_REMINDERS = {
    "/talks/": "then run /visual-audit to score with discussant",
}
...
def get_extra_reminder(file_path: str) -> str:
    """Return extra reminder text for specific directories."""
    normalized = file_path.replace("\\", "/")
    for dir_pattern, reminder in EXTRA_REMINDERS.items():
...
def format_reminder(file_path: str, action: str) -> str:
    ...
    extra = get_extra_reminder(file_path)
    extra_line = f"\n   → {YELLOW}{extra}{NC}" if extra else ""
    return f"""
{CYAN}Verification reminder:{NC} {filename}
   → {GREEN}{action}{NC} before marking task complete{extra_line}
"""
